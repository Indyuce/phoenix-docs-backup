...
![](uploads/gem_stone_color.png)

![](uploads/item_socket_color.png)
...
To remove a gemstone from an item, a consumable would have to be created featuring the _Random Unsocket_ option. Setting this option to ``1`` will remove 1 random gemstone from the item. To remove all of the gems off the item, simply set the value to something higher.

![](uploads/random_unsocket.png)

To prevent the consumable from accidentally being eaten instead of applied to the item, you may enable the `inedible` option. This way, the consumable will only work when drag & dropped onto an item inside the player inventory.
```yml
GEMSTONE_REMOVER:
  base:
    material: FLINT
