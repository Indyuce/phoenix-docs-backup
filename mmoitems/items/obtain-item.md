...
``/mi generate`` lets you generate a random item based on specific criterias that you give as command arguments. The command format is the following:

```txt
/mi generate <player> (extra-args)
```
...
The available optional command arguments are listed below:
...
| ``-gimme``  | Adds the item to your inventory instead. |
...
| ``-type:<item-type>``   | Choose the [item type](../features/types.md).     |
| ``-id:<id>``| Use that if you want a specific [item template](templates.md) to be selected.   |
...
```txt
/mi generate <player-level> -matchlevel -matchclass -tier:rare
```
...
More information in the official [MythicMobs wiki](https://git.lumine.io/mythiccraft/MythicMobs/-/wikis/drops/Drops).
...
## Using MMOItems drop tables

Please read [this wiki page](drop-tables.md).
