...
## Custom skills
You can register custom skills inside of all of the MMO plugins using well known plugins featuring a custom scripting language:
- [MythicMobs](Using MythicMobs)
- [SkillAPI](Using%20other%20plugins#using-skillapi)
- [MythicLib](https://gitlab.com/phoenix-dvpmt/mythiclib/-/wikis/Using-other-plugins#using-mythiclib)
...
Nevertheless, a few skills are specific to either MMOItems or MMOCore because they utilize features which aren't available when the plugin is disabled. For instance, _Neptune's Gift_ (increases mana regeneration when in water) isn't available when MMOCore is not installed.

Here is the full list of the default MythicLib skills. **Tip: use CTRL+F when looking for an ability with certain keywords.**
...
| Wither          | Temporarily withers your opponent.                       | duration, amplifier                 |
