...
### Register this script into MythicLib
...
### Configuring your skill in MMOItems/MMOCore
...
which will always deal a static amount of 10 damage, use:

```yaml
  - damage{a="<modifier.damage>"}
```

Since recent MythicMobs builds also support math formulas, you can also use:

```yaml
  - potion{type=SLOW;duration="<modifier.duration> * 20";lvl=2}
```

This will take the `duration` modifier from MMOItems/MMOCore and multiply it by 20! That way, the modifier is displayed as seconds but you will be providing ticks to the `potion` MythicMobs mechanic.

Here is the full list of placeholders implemented by MythicLib.
...
### Dealing damage using a MM skill

Use the following damaging mechanic:
...
This has the effect of multiplying the current SKILL damage by a specific factor. Currently this factor is `1 + <modifier.extra> / 100` so if the `extra` skill modifier is set to 50, SKILL damage will be increased by 50%. You can use this mechanic to increase incoming damage dealt by entities or players by a certain factor (e.g increase damage when low on health).
